/*
===============================================================================
Load Script: Silver Layer (Bronze -> Silver) — PostgreSQL
===============================================================================
Script Purpose:
  Transforms and loads data from bronze into the silver schema.
  - Truncates silver tables before loading
  - Cleanses, standardizes, and deduplicates source data

Prerequisites:
  - Bronze tables loaded (CALL bronze.load_bronze())
  - Silver tables already created (scripts/silver/ddl_silver.sql)
    ...
    SELECT
        ...
        CAST(
            LEAD(cpi.prd_start_dt) OVER (
                PARTITION BY cpi.prd_key
                ORDER BY cpi.prd_start_dt
            ) - INTERVAL '1 day'
            AS DATE
        ) AS prd_end_dt
